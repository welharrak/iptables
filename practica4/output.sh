...
iptables -A OUTPUT -d 172.18.0.0/24 -p tcp --dport 7 -j ACCEPT

# CAS 3: podem accedir a qualsevol port 13, menys del host 172.18.0.3 
iptables -A OUTPUT 172.18.0.3 -p tcp --dport 13 -d -j DROP
iptables -A OUTPUT -p tcp --dport 13 -j ACCEPT

# CAS 4: nomès poder accedir al port 7 de host 172.18.0.4
iptables -A OUTPUT 172.18.0.4 -p tcp --dport 7 -d -j ACCEPT
...
iptables -A OUTPUT 0.0.0.0/0 -p tcp --dport 7 -d -j ACEEPT